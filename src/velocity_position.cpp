...
namespace srs_mavros_plugins
{
  ...
  {
  public:
    explicit VelocityPositionPlugin(mavros::plugin::UASPtr uas_)
        : Plugin(uas_, "velocity_position")
    {
      ...
      node_declare_and_watch_parameter(
          ...
          { tf_frame_id_ = p.as_string(); });
      node_declare_and_watch_parameter(
          "tf.child_frame_id", "base_link", [&](const rclcpp::Parameter &p)
          { tf_child_frame_id_ = p.as_string(); });
        ...
        mavros::plugin::filter::SystemAndOk filter [[maybe_unused]])
    {
      ...
      if (imu_rpy_enu_) {
        ...
        body_twist.linear.x = std::cos(-euler_enu_yaw) * vel_enu_x - std::sin(-euler_enu_yaw) * vel_enu_y;
        body_twist.linear.y = std::sin(-euler_enu_yaw) * vel_enu_x + std::cos(-euler_enu_yaw) * vel_enu_y;
      ...
      if(imu_gyro_flu_) {
        ...
        if (wheel_yaw_rate_) {
          ...
          const float gyro_marge_rate = std::min(std::max((std::fabs(gyro_yaw_rate) - 0.02f) / (0.5f - 0.02f), 0.0f), 1.0f);
          body_twist.angular.z = gyro_marge_rate * gyro_yaw_rate + (1 - gyro_marge_rate) * wheel_yaw_rate_.value();
        } else {
      ...
      if (last_stamp_) {
        ...
        odom.pose.pose = generateVelocityPose3D(last_pose_2d_, pose_enu_z, euler_enu_roll, euler_enu_pitch);
  ...
  private:
    static geometry_msgs::msg::Pose2D update2dPose(const geometry_msgs::msg::Pose2D &pose_2d_enu, const geometry_msgs::msg::Twist &twist_2d_frd, const float dt)
    {
      float vpos_half_yaw = pose_2d_enu.theta + twist_2d_frd.angular.z * dt / 2.0f;
      float vpos_vel_x = std::cos(vpos_half_yaw) * twist_2d_frd.linear.x - std::sin(vpos_half_yaw) * twist_2d_frd.linear.y;
      float vpos_vel_y = std::sin(vpos_half_yaw) * twist_2d_frd.linear.x + std::cos(vpos_half_yaw) * twist_2d_frd.linear.y;
    ...
    static geometry_msgs::msg::Pose generateVelocityPose3D(const geometry_msgs::msg::Pose2D &pose_2d_enu, const float pos_z, const float roll, const float pitch) {
...
// tf2::fromMsg(msg_quat, tf2_quat_from_msg);
    ...
    geometry_msgs::msg::TransformStamped generateTransform(nav_msgs::msg::Odometry & odom) {
    ...
    void print(const geometry_msgs::msg::Pose &enu_pose, const geometry_msgs::msg::Twist &enu_twist, const float dt)
    {
      RCLCPP_WARN(get_logger(), "diff %f", dt);
      RCLCPP_WARN(get_logger(), "%f %f %f", enu_pose.position.x, enu_pose.position.y, enu_pose.position.z);
      ...
      RCLCPP_WARN(get_logger(), "%f %f %f", enu_twist.linear.x, enu_twist.linear.y, enu_twist.linear.z);
      RCLCPP_WARN(get_logger(), "%f %f %f", enu_twist.angular.x, enu_twist.angular.y, enu_twist.angular.z);    
    }
